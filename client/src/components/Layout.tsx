...
<p>सरकारी नौकरी | Sarkari Naukri | Government Jobs India | Free Job Alert | Rojgar Samachar | नौकरी</p>
  ...
  // Search API — used by search popup and /search page
  app.get("/api/search", async (req, res) => {
    try {
      ...
      if (!q || q.length < 2) {
        return res.json({ data: [], total: 0, page: 1, limit, totalPages: 0 });
      }

      const result = await storage.searchPosts(q, page, limit);
      res.json(result);
    } catch (error) {
